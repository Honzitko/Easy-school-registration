...
class ESR_Ajax_Worker {
	...
	public function edit_course_registration_callback( $data ) {
		...
		$student        = get_user_by( 'email', trim( $data['student_email'] ) );
		$dancing_with   = get_user_by( 'email', trim( $data['dancing_with'] ) );
		$partner        = get_user_by( 'email', trim( $data['partner_email'] ) );
		$new_dancing_as = $data['dancing_as'];
		...
		$wpdb->update( $wpdb->prefix . 'esr_course_registration', [
			'user_id'      => $student->ID,
			'dancing_with' => $data['dancing_with'] !== '' ? $data['dancing_with'] : null,
		...
		$dancing_with_email = trim( $data['dancing_with'] );
	...
	public function change_course_passed( $course_id, $passed ) {
		if ( $course_id ) {
			...
			return [
				'status' => $passed ? __( 'Passed', 'easy-school-registration' ) : __( 'Active', 'easy-school-registration' )
			];
		}
		...
		if ( $user_email && $payment_status ) {
			...
			if ( ( $price !== null ) || ( $payment_status === 'not_paying' ) ) {
				do_action( 'esr_save_user_payment', [
					...
					'note'               => isset( $data['note'] ) ? htmlspecialchars( $data['note'] ) : null,
					...
					'user_id' => $student->ID,
					'wave_id' => (int) $data['wave_id'],
				], ESR()->payment->get_payment_by_wave_and_user( (int) $data['wave_id'], $student->ID ) );
			}

			if ( isset( $data['esr_payment_email_confirmation'] ) && filter_var( $data['esr_payment_email_confirmation'], FILTER_VALIDATE_BOOLEAN ) ) {
				do_action( 'esr_send_payment_confirmation_email', (int) $data['wave_id'], $user_email );
			}

			$new_status = ESR()->payment_status->get_status( ESR()->payment->get_payment_by_wave_and_user( (int) $data['wave_id'], $student->ID ) );

			return [
				'payment'              => ESR()->currency->prepare_price( is_int( $data['payment'] ) ? $price : ( ! empty( $data['payment'] ) ? $data['payment'] : 0 ) ),
				'payment_status'       => $new_status,
				'payment_status_title' => ESR()->payment_status->get_title( $new_status ),
				'price'                => is_int( $data['payment'] ) ? $price : ( ! empty( $data['payment'] ) ? $data['payment'] : 0 ),
				'payment_type'         => ESR()->payment_type->get_title( $data['payment_type'] ),
				'payment_note'         => isset( $data['note'] ) ? htmlspecialchars( $data['note'] ) : ''
			];
		}
	...
	public function esr_load_student_data( $user_id ) {
		...
		if ( $user_id ) {
			...
			if ( $user ) {
				$user_data['data']['name']  = $user->display_name;
				$user_data['data']['email'] = $user->user_email;
			}

			$user_data['note'] = get_user_meta( $user_id, 'esr_student_note', true );
			$user_data['user_id'] = $user_id;

			if ( $user_meta ) {
				$user_data['data']['phone'] = isset( $user_meta['esr-course-registration-phone'] ) ? $user_meta['esr-course-registration-phone'][0] : '';
			}
