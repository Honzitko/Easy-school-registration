...
class ESR_Registration_Worker {
	...
	private function course_registration_validation( $data ) {
		...
		if ( $this->check_required( 'name', $data ) ) {
			$esr_reg_errors->add( 'user_info.name', __( 'Name is required.', 'easy-school-registration' ) );
		}
		if ( $this->check_required( 'surname', $data ) ) {
			$esr_reg_errors->add( 'user_info.surname', __( 'Surname is required.', 'easy-school-registration' ) );
		}
		if ( $this->check_required( 'email', $data ) ) {
			$esr_reg_errors->add( 'user_info.email.required', __( 'Email is required.', 'easy-school-registration' ) );
		} else {
			if ( ! is_email( $data->user_info->email ) ) {
				$esr_reg_errors->add( 'user_info.email.invalid', __( 'Email is not valid', 'easy-school-registration' ) );
			}
		}
		if ( ( intval( ESR()->settings->esr_get_option( 'show_phone_input', 1 ) ) !== - 1 ) && ( intval( ESR()->settings->esr_get_option( 'phone_required', 1 ) ) !== - 1 ) && $this->check_required( 'phone', $data ) ) {
			$esr_reg_errors->add( 'user_info.phone', __( 'Phone is required.', 'easy-school-registration' ) );
		}
		if ( ! isset( $data->courses ) || ! $data->courses ) {
			$esr_reg_errors->add( 'courses.all.empty', __( 'You must choose at least one course.', 'easy-school-registration' ) );
		} else {
			foreach ( $data->courses as $course_id => $course ) {
				if ( ! ESR()->course->is_course_solo( intval( $course_id ) ) ) {
					...
					if ( ( isset( $course->choose_partner ) && ( intval( $course->choose_partner ) === 1 ) ) || ESR()->course->is_partner_enforcement_enabled( intval( $course_id ) ) ) {
						if ( $dancing_with === '' ) {
							$esr_reg_errors->add( 'courses.' . $course_id . '.email_not_filled', __( 'Partner email is not filled', 'easy-school-registration' ) );
						}
						if ( ! is_email( $dancing_with ) ) {
							$esr_reg_errors->add( 'courses.' . $course_id . '.email_invalid', __( 'Partner email is not valid', 'easy-school-registration' ) );
						}
						if ( isset( $data->user_info->email ) && ( trim( $data->user_info->email ) == $dancing_with ) ) {
							$esr_reg_errors->add( 'courses.' . $course_id . '.partner_email', __( 'Please write your partner email not yours', 'easy-school-registration' ) );
						}
					} else {

						if ( ESR()->course->is_partner_enforcement_enabled( intval( $course_id ) ) && ( $dancing_with === '' ) ) {
							$esr_reg_errors->add( 'courses.' . $course_id . '.partner_required', __( 'Partner is required for this course', 'easy-school-registration' ) );
						}
					}
					if ( ! isset( $course->dancing_as ) || ( $course->dancing_as === '' ) || ( $course->dancing_as === null ) ) {
						$esr_reg_errors->add( 'courses.' . $course_id . '.dancing_as_invalid', __( 'Please choose Dancing as', 'easy-school-registration' ) );
					}
				}
			}
		}
		...
		if ( isset( $data->courses ) ) {
			...
			foreach ( $data->courses as $course_id => $course ) {
				...
				if ( ! in_array( $course_data->wave_id, $enabled_waves ) ) {
					$esr_reg_errors->add( 'courses.' . $course_id . '.closed', __( 'Registration for this waves was closed', 'easy-school-registration' ) );
				}
				...
				if ( ! $test_full ) {
					$esr_reg_errors->add( 'courses.' . $course_id . '.full', __( 'Course is full', 'easy-school-registration' ) );
				}

				if ( $user_id !== null ) {
					if ( ESR()->course->is_course_already_registered( $course_id, $user_id ) ) {
						$esr_reg_errors->add( 'courses.' . $course_id . '.already_registered', __( 'You have already registered this course', 'easy-school-registration' ) );
					}
				}
