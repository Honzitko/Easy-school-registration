...
class ESR_Add_Over_Limit_Worker {
	...
	public function process_form($data) {
		if (isset($data['esr_add_over_limit_submit'])) {
			...
			$course_data = ESR()->course->get_course_data($data['esr_course_id']);

			if ($leader && !ESR()->course->is_course_already_registered($data['esr_course_id'], $leader->ID)) {
				$registration_data = [];
				if (!$course_data->is_solo) {
					$registration_data['reg_id'] = $this->add_registrations($data['esr_course_id'], $leader, $follower, ESR_Dancing_As::LEADER, isset($data['esr_leader_free_registration']));
				} else {
					$registration_data['reg_id'] = $this->add_registrations($data['esr_course_id'], $leader, null, ESR_Dancing_As::SOLO, isset($data['esr_leader_free_registration']));
				}
				...
				$this->update_summary($data['esr_course_id'], ESR_Dancing_As::LEADER);
				ESR()->email->send_course_registration_email($registration_data);
			}

			if ($follower && !ESR()->course->is_course_already_registered($data['esr_course_id'], $follower->ID)) {
				$registration_data = [];
				if (!$course_data->is_solo) {
					$registration_data['reg_id'] = $this->add_registrations($data['esr_course_id'], $follower, $leader, ESR_Dancing_As::FOLLOWER, isset($data['esr_follower_free_registration']));
				} else {
					$registration_data['reg_id'] = $this->add_registrations($data['esr_course_id'], $follower, null, ESR_Dancing_As::SOLO, isset($data['esr_follower_free_registration']));
				}
				...
				$this->update_summary($data['esr_course_id'], ESR_Dancing_As::FOLLOWER);
				ESR()->email->send_course_registration_email($registration_data);
			}
		}
	}
	...
	public function add_user($data, $key_email, $key_name, $key_surname, $key_phone) {
		if (isset($data[$key_email]) && $data[$key_email]) {
			$user_email = $data[$key_email];
			$user       = get_user_by('email', $user_email);
			if (!$user) {
				$user = ESR()->user->create_new_user($user_email, $data[$key_name], $data[$key_surname]);

				if (isset($data[$key_phone]) && $data[$key_phone]) {
					update_user_meta($user->ID, 'esr-course-registration-phone', $data[$key_phone]);
				}
			}
