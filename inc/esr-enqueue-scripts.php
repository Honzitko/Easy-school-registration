...
class ESR_Enqueue_Scripts {
	...
	public static function esr_register_theme_scripts() {
		if ( intval( ESR()->settings->esr_get_option( 'preload_styles', - 1 ) ) === - 1 ) {
			...
			wp_register_script( 'esr_page_script', ESR_PLUGIN_URL . 'inc/assets/web/js/esr-script.js', [ 'jquery' ], ESR_VERSION );
			wp_register_script( 'esr_spin_js_script', ESR_PLUGIN_URL . 'libs/spin/js/spin.js', [ 'jquery' ], ESR_VERSION );
			wp_localize_script( 'esr_page_script', 'esr_ajax_object', [ 'ajaxurl' => admin_url( 'admin-ajax.php' ) ] );
		} else {
			if ( intval( ESR()->settings->esr_get_option( 'disable_styles', - 1 ) ) !== 1 ) {
				wp_enqueue_style( 'esr_page_style', ESR_PLUGIN_URL . 'inc/assets/web/css/esr-style.css', [], ESR_VERSION );
			}
			wp_enqueue_script( 'esr_page_script', ESR_PLUGIN_URL . 'inc/assets/web/js/esr-script.js', [ 'jquery' ], ESR_VERSION );
			wp_enqueue_script( 'esr_spin_js_script', ESR_PLUGIN_URL . 'libs/spin/js/spin.js', [ 'jquery' ], ESR_VERSION );
			wp_localize_script( 'esr_page_script', 'esr_ajax_object', [ 'ajaxurl' => admin_url( 'admin-ajax.php' ) ] );
		}
	}
	...
	public static function esr_scripts_datatable_callback() {
		wp_enqueue_script( 'esr_dataTables_script', ESR_PLUGIN_URL . 'libs/datatable/js/jquery.dataTables.js', [ 'jquery' ], ESR_VERSION );
		wp_enqueue_style( 'esr_dataTables_bootstrap_style', ESR_PLUGIN_URL . 'libs/datatable/css/dataTables.bootstrap.css', [], ESR_VERSION );
	}
	...
	public static function esr_scripts_notify_callback() {
		wp_enqueue_script( 'esr_notify_script', ESR_PLUGIN_URL . 'libs/bootstrap-notify.min.js', [ 'jquery' ], ESR_VERSION );
	}
...
add_action( 'esr_scripts_notify', [ 'ESR_Enqueue_Scripts', 'esr_scripts_notify_callback' ] );
