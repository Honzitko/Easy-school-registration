		...
		<div class="wrap">
			<?php if ($user_payments) { ?>
				<div class="esr_user_settings payments">
					<h2><?php _e('Payments', 'easy-school-registration'); ?></h2>
					<table id="datatable" class="table table-default table-bordered esr-datatable">
						...
						foreach ($user_payments as $id => $payment) {
							...
							?>
							<tr>
								<td><?php echo $wave_data->title ?></td>
								<td><?php echo ESR()->payment_status->get_title($paid_status); ?></td>
								<td><?php echo ESR()->currency->prepare_price($payment['to_pay']); ?></td>
								<?php if (intval(ESR()->settings->esr_get_option('show_already_paid', -1)) != -1) { ?>
									<td><?php echo ESR()->currency->prepare_price($payment['payment'] ? $payment['payment'] : 0); ?></td>
								<?php } ?>
							</tr>
							<?php
						}
