			...
			<div class="esr-schedule-calendar esr-clearfix">
				<div class="esr-row esr-header">
					...
					foreach ($courses_times as $key => $value) {
						echo '<div class="esr-time">' . $key . '</div>';
					}
					?>
				</div>
				<?php
				foreach ($courses_by_day as $day_id => $halls_schedule) {
					?>
					<div class="esr-row">
						<div class="esr-day"><?php echo ESR()->day->get_day_title($day_id); ?></div>
								...
								foreach ($courses_times as $key => $value) {
									if (isset($schedule[$key])) {
										...
										<div class="esr-course esr-add">
											...
											<span class="esr-sub-title"><?php echo $course->sub_header; ?></span>
											<span class="esr-level"><?php echo ($course->level_id ? '(' . ESR()->course_level->get_title($course->level_id) . ')' : '') . ' *' . substr(ESR()->course_group->get_title($course->group_id), 0, 1); ?></span>
											<span class="esr-teachers"><?php echo ($course->teacher_first ? ESR()->teacher->get_teacher_name($course->teacher_first) : '') . ($course->teacher_second ? ($course->teacher_second ? ' & ' : '') . ESR()->teacher->get_teacher_name($course->teacher_second) : ''); ?></span>
										</div>
									<?php } else { ?>
