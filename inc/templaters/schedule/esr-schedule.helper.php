...
class ESR_Schedule_Helper {
	...
	public function print_course_html( $course, $for_registration, $position = 'width', $position_enabled = true ) {
		...
		<div
			...
			<?php } ?>
				class="<?php echo implode( ' ', $course_classes ); ?>" <?php if ( $for_registration ) { ?>
				data-group="<?php echo $course->group_id; ?>"
				data-level="<?php echo $course->level_id; ?>"
				data-id="<?php echo $course->id; ?>"
				data-wave="<?php echo $course->wave_id; ?>"
				data-day="<?php echo ESR()->day->get_day_title( $course->day ); ?>"
				data-start="<?php echo $course->time_from; ?>"
				data-price="<?php echo $course->real_price; ?>"
				data-enforce-partner="<?php echo $course->enforce_partner; ?>"
			...
			<?php }
				...
				data-group="<?php echo $course->group_id; ?>"
				data-level="<?php echo $course->level_id; ?>"><?php } ?>
			<span class="esr-title"><?php echo stripslashes( $course->title ); ?></span>
			<span class="esr-sub-title"><?php echo( $course->sub_header ? $course->sub_header : '' ); ?></span>
			<span class="esr-course-hide-hover">
				<span class="esr-sub-title"><?php echo $course->time_from . ' - ' . $course->time_to; ?></span>
				<span class="esr-teachers"><?php echo ESR()->teacher->get_teachers_names( $course->teacher_first, $course->teacher_second ); ?></span>
			</span>
			...
			<?php if ( $course->is_solo ) { ?>
				<span class="esr-count">
							<?php
							echo __( 'Students', 'easy-school-registration' ) . ': ';
							if ( $course->settings['hover_option'] === 'registrations' ) {
								echo $solo_registration_enabled ? ( $summary->registered_solo . '/' . $summary->max_solo ) : __( 'Full', 'easy-school-registration' );
							} else {
								echo $solo_registration_enabled ? __( 'left', 'easy-school-registration' ) . ' ' . $summary->sl : __( 'Full', 'easy-school-registration' );
							}
							?>
						</span>
			<?php } else { ?>
				<span class="esr-count">
							...
							<?php
							echo __( 'Leaders', 'easy-school-registration' ) . ': ';
							if ( $course->settings['hover_option'] === 'registrations' ) {
								echo $leader_registration_enabled ? ( $summary->registered_leaders . '/' . $summary->max_leaders ) : __( 'Full', 'easy-school-registration' );
							} else {
								echo $leader_registration_enabled ? __( 'left', 'easy-school-registration' ) . ' ' . $summary->ll : __( 'Full', 'easy-school-registration' );
							}
							?>
					</span>
				<span class="esr-count">
							<?php
							echo __( 'Followers', 'easy-school-registration' ) . ': ';
							if ( $course->settings['hover_option'] === 'registrations' ) {
								echo $follower_registration_enabled ? ( $summary->registered_followers . '/' . $summary->max_followers ) : __( 'Full', 'easy-school-registration' );
							} else {
								echo $follower_registration_enabled ? __( 'left', 'easy-school-registration' ) . ' ' . $summary->fl : __( 'Full', 'easy-school-registration' );
							}
							?>
					</span>
			<?php } ?>
	...
	public function print_mobile_course_html( $course, $for_registration ) {
		...
		$course_classes = $this->esr_get_course_classes( $course, $for_registration, $course_enabled );
		?>
		<li class="<?php echo implode( ' ', $course_classes ); ?>"
			...
			<span class="esr-course-info">
				...
				?>
				<span class="esr-teachers"><?php echo ( strpos( $teacher_string, ' & ' ) !== false ? __( 'Teachers', 'easy-school-registration' ) : __( 'Teacher', 'easy-school-registration' ) ) . ': ' . $teacher_string; ?></span>
			</span>
			<span class="esr-time"><?php echo $course->time_from . ' - ' . $course->time_to; ?></span>
			<?php do_action( 'esr_registration_schedule_print', $course, $course_enabled, $for_registration ); ?>
		</li>
		<?php
	}
	...
	<div class="esr-course esr-empty"
		<?php if ( $position_enabled ) { ?>
			style="<?php echo $position . ':' . $this->get_time_width( $end_time, $start_time ) . 'px;'; ?>"
		<?php } ?>></div><?php
	...
	public function print_wave_closed_text( $wave_id ) {
		if ( ESR()->wave->is_wave_registration_not_opened_yet( $wave_id ) ) {
			...
			echo nl2br( stripcslashes( $to_return ) );
			?></div><?php
		} elseif ( ESR()->wave->is_wave_registration_closed( $wave_id ) ) {
			?>
			<div class="esr-registration-closed"><?php
			echo nl2br( stripcslashes( ESR()->settings->esr_get_option( 'registration_closed' ) ) );
			?></div><?php
		}
	}
