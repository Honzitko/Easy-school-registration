			...
			<div class="esr-level-filters">
				...
				foreach ( $wave_ids as $wave_id ) {
					if ( ! $for_registration || ESR()->wave->is_wave_registration_active( $wave_id ) || ( isset( $settings['test'] ) && ( intval( $settings['test'] ) === 1 ) ) ) {
						...
						?>
					<div class="esr-level-filter<?php echo( isset( $settings['level_filter_hide_courses'] ) && $settings['level_filter_hide_courses'] ? ' esr-level-filter-hide' : ' esr-level-filter-fade' ); ?>" data-wave="<?php echo $wave_id ?>">
						<span class="esr-level-filter-button" data-level-id="all"><?php _e( 'All', 'easy-school-registration' ); ?></span>
						<?php
						foreach ( $wave_levels as $level_id ) {
							if ( isset( $levels[ $level_id['level_id'] ] ) ) {
								?>
								<span class="esr-level-filter-button esr-level-<?php echo $level_id['level_id']; ?>" data-level-id="<?php echo $level_id['level_id']; ?>"><?php echo $levels[ $level_id['level_id'] ]; ?></span>
								<?php
							}
						}
						?></div><?php
