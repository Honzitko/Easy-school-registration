...
class ESR_Settings_Tag_Templater {
		...
		if ($courses) {
			...
			foreach ($courses as $id) {
				...
				if (intval(ESR()->settings->esr_get_option('multiple_dates', -1)) === -1) {
					$replacement .= "<li>" . $course_data->title . " - " . ESR()->day->get_day_title($course_data->day) . " (" . $course_data->time_from . ' / ' . $course_data->time_to . ")" . "</li>";
				} else {
					$replacement .= "<li>" . $course_data->title;
					$replacement .= "<ul>";
					foreach (ESR()->multiple_dates->esr_get_all_course_dates($course_data->id) as $key => $date) {
						$replacement .= "<li>" . ESR()->day->get_day_title($date->day) . " (" . $date->time_from . ' / ' . $date->time_to . ")" . "</li>";
					}
					$replacement .= "</ul></li>";
				}
			}
	...
	public static function esr_tag_replace_wave_title($tag, $body, $wave_id) {
		...
		return str_replace("[" . $tag['tag'] . "]", $wave->title, $body);
	}
	...
	public static function esr_tag_replace_waves_title($tag, $body, $wave_ids) {
		...
		return str_replace("[" . $tag['tag'] . "]", implode(', ', $waves_title), $body);
	}


	public static function esr_tag_replace_price($tag, $body, $price) {
		return str_replace("[" . $tag['tag'] . "]", ESR()->currency->prepare_price($price), $body);
	}


	public static function esr_tag_replace_price_difference($tag, $body, $payment) {
		return str_replace("[" . $tag['tag'] . "]", ESR()->currency->prepare_price($payment->to_pay - $payment->payment), $body);
	}
	...
	public static function esr_tag_replace_settings_parameter($tag, $body) {
		return str_replace("[" . $tag['tag'] . "]", ESR()->settings->esr_get_option($tag['id'], null), $body);
	}


	public static function esr_tag_replace_variable_symbol($tag, $body, $variable_symbol) {
		return str_replace("[" . $tag['tag'] . "]", $variable_symbol, $body);
	}
	...
	public static function esr_tag_replace_course_teachers($tag, $body, $course) {
		return str_replace("[" . $tag['tag'] . "]", ESR()->teacher->get_teachers_names($course->teacher_first, $course->teacher_second), $body);
	}


	public static function esr_tag_replace_course_day($tag, $body, $course) {
		return str_replace("[" . $tag['tag'] . "]", ESR()->day->get_day_title($course->{$tag['id']}), $body);
	}
	...
	public static function esr_tag_replace_course_days_times($tag, $body, $days_times) {
		...
		foreach ($days_times as $key => $day_data)
		{
			...
			$replacement .= "<li>" . ESR()->day->get_day_title($day_data->day) . " (" .$time_from->format(get_option('time_format')) . " / " . $time_to->format(get_option('time_format')) . ")</li>";
		}

		return str_replace("[" . $tag['tag'] . "]", $replacement !== "" ? "<ul>" . $replacement . "</ul>" : "", $body);
	}


	public static function esr_tag_replace_dancing_as($tag, $body, $dancing_as) {
		return str_replace("[" . $tag['tag'] . "]", ESR()->dance_as->get_title($dancing_as), $body);
	}
	...
	public static function esr_tag_replace_registration_course_list($tag, $body, $courses) {
		...
		if ($courses) {
			...
			foreach ($courses as $course_id => $course) {
				...
				$replacement .= "<li>" . $course_data->title . "</li>";
			}
			$replacement .= "</ul>";
			...
			foreach ($courses as $course_id => $course) {
				...
				if ((isset($course->dancing_with) && $course->dancing_with) || (isset($course->dancing_as) && !ESR()->dance_as->is_solo($course->dancing_as))) {
					...
					if (isset($course->dancing_as) && !ESR()->dance_as->is_solo($course->dancing_as)) {
						$replacement .= "<li>" . __('Role', 'easy-school-registration') . ": " . (ESR()->dance_as->get_title($course->dancing_as)) . "</li>";
					}
					if (isset($course->dancing_with) && $course->dancing_with) {
						$replacement .= "<li>" . __('Partner', 'easy-school-registration') . ": " . $course->dancing_with . "</li>";
					}
					$replacement .= "</ul>";
				}
				$replacement .= "</li>";
