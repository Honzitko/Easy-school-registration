...
class ESR_Wave_Courses_Select_Template {
	...
	public static function esr_print_select() {
		...
		?>
		<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" class="esr-select-course">
			<span><?php echo __('Select Course', 'easy-school-registration') . ': '; ?></span>
			<select name="esr_course">
				<?php
				foreach ($courses as $key => $course) {
					?>
					<option
					value="<?php echo $course->id ?>" <?php if ($course->id == $selected_course) { ?>selected="selected"<?php } ?>>
					<?php echo stripslashes($course->title) . ' - ' . ESR()->day->get_day_title($course->day) . ' (' . $course->time_from . '/' . $course->time_to .')'; ?>
					</option><?php
				}
				?>
			</select>
			<input type="submit" name="esr_choose_course_submit" class="page-title-action"  value="<?php _e('Select', 'easy-school-registration'); ?>">
		</form>
		<?php
	}
	...
	public static function esr_get_selected_course($wave_id) {
		...
		if (isset($_POST['esr_choose_course_submit']) && isset($_POST['esr_course'])) {
			update_user_meta(get_current_user_id(), 'esr_user_course_id', $_POST['esr_course']);
			return $_POST['esr_course'];
