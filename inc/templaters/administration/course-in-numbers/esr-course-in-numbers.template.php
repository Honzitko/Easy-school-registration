	...
	public static function print_page() {
		...
		<div class="wrap esr-settings esr-course-in-numbers">
			...
			<div class="esr-settings-header esr-clearfix">
				<h2 class="esr-title"><?php _e('Course In Numbers', 'easy-school-registration'); ?></h2>
				<?php if (current_user_can('esr_course_in_number_edit')) { ?>
					<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" class="esr-recount-button">
						<input type="hidden" name="esr_recount_wave_id" value="<?php echo $selected_wave; ?>">
						<input type="submit" name="esr_recount_wave" class="page-title-action" value="Recount Statistics">
					</form>
				<?php } ?>
			</div>
			<div class="content">
				...
				if (ESR()->settings->esr_get_option('courses_schedule_style', 'as_table') === 'as_table') {
					...
					$templater_schedule->print_content($selected_wave, [], false, new ESR_Courses_In_Numbers_Course_Helper());
				}
				?>
			</div>
		</div>
