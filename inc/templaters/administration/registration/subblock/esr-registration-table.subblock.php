...
class ESR_Registrations_Table_Subblock_Templater {
	...
	public function print_table( $selected_wave ) {
		...
		$classes = [ 'table table-default table-bordered esr-registered-courses esr-datatable esr-copy-table esr-excel-export esr-enable-scroll' ];
		...
		<table id="datatable" class="<?php echo implode( ' ', $classes ) ?>">
			<thead>
			<tr>
				<th class="esr-filter-disabled" data-key="esr_reg_date"><?php _e( 'Timestamp', 'easy-school-registration' ); ?></th>
				<?php if ( $user_can_edit ) { ?>
					<th id="esr_reg_actions" class="esr-hide-print esr-filter-disabled no-sort"
					    data-key="esr_reg_actions" data-visibility="1"><?php _e( 'Actions', 'easy-school-registration' ); ?></th>
				<?php } ?>
				<th class="esr-filter-disabled esr-header-note esr-note"><?php _e( 'Note', 'easy-school-registration' ); ?>
				...
				<?php if ( $show_user_note_enabled ) { ?>
					<th class="esr-filter-disabled esr-header-note esr-user-note"><?php _e( 'User Note', 'easy-school-registration' ); ?>
				...
				<?php if ( $user_show_emails ) { ?>
					<th class="esr-filter-disabled esr-student-email"><?php _e( 'Email', 'easy-school-registration' ); ?></th>
				<?php } ?>
				<?php if ( $show_phone ) { ?>
					<th class="esr-filter-disabled esr-student-phone"><?php _e( 'Phone', 'easy-school-registration' ); ?></th>
				<?php } ?>
				<th class="no-sort course"><?php _e( 'Course', 'easy-school-registration' ); ?></th>
				<th class="no-sort esr-multiple-filters"><?php _e( 'Day', 'easy-school-registration' ); ?></th>
				<th class="no-sort esr-multiple-filters"><?php _e( 'Time', 'easy-school-registration' ); ?></th>
				<th class="no-sort"><?php _e( 'Dancing Role', 'easy-school-registration' ); ?></th>
				<?php if ( $user_show_emails ) { ?>
					<th class="esr-filter-disabled"><?php _e( 'Registered Partner', 'easy-school-registration' ); ?></th>
				<?php } ?>
				<th class="esr-filter-disabled"><?php _e( 'Paired Partner', 'easy-school-registration' ); ?></th>
				<?php if ( intval( ESR()->settings->esr_get_option( 'free_registrations_enabled', - 1 ) ) != - 1 ) { ?>
					<th class="no-sort"><?php _e( 'Free Registration', 'easy-school-registration' ); ?></th>
				<?php } ?>
				<?php if ( $show_payment_enabled ) { ?>
					<th class="payment-status"><?php _e( 'Payment Status', 'easy-school-registration' ); ?></th>
				<?php } ?>
			...
			foreach ( $registrations as $registration_id => $registration ) {
				...
				$student_name  = isset( $user_data[ $registration->user_id ] ) ? $user_data[ $registration->user_id ]->display_name : __( 'deleted student', 'easy-school-registration' );
				$student_email = isset( $user_data[ $registration->user_id ] ) ? $user_data[ $registration->user_id ]->user_email : __( 'deleted student', 'easy-school-registration' );
				...
				if ( $registration->partner_id != null ) {
					$partner_name  = isset( $user_data[ $registration->partner_id ] ) ? $user_data[ $registration->partner_id ]->display_name : __( 'deleted student', 'easy-school-registration' );
					$partner_email = isset( $user_data[ $registration->partner_id ] ) ? $user_data[ $registration->partner_id ]->user_email : __( 'deleted student', 'easy-school-registration' );
				}
				...
				<tr
					...
					<?php if ( $user_can_edit ) { ?>
						...
						data-course-id="<?php echo $registration->course_id; ?>"
						data-student-name="<?php echo $student_name; ?>"
						data-student-email="<?php echo $student_email; ?>"
						data-partner-email="<?php echo $partner_email; ?>"
						data-dancing-with="<?php echo $registration->dancing_with; ?>"
						data-partner-name="<?php echo $partner_name; ?>"
						data-dancing-as="<?php echo $registration->dancing_as; ?>"
						<?php do_action( 'esr_template_registration_row_data', $registration ); ?>
					<?php } ?>>
					<td class="registration-date"><?php echo $registration->time; ?></td>
					<?php if ( $user_can_edit ) { ?>
					...
					<td class="esr-note">
						<?php if ( ( $registration->note !== null ) && ( $registration->note !== "" ) ) { ?>
							<span class="dashicons dashicons-admin-comments esr-show-note" title="<?php echo htmlspecialchars( $registration->note, ENT_QUOTES, 'UTF-8' ); ?>"></span>
							<span class="dashicons dashicons-welcome-comments esr-hide-note"></span>
							<span class="esr-note-message"><?php echo htmlspecialchars( $registration->note, ENT_QUOTES, 'UTF-8' ); ?></span>
						<?php } ?>
					</td>
					<?php if ( $show_user_note_enabled ) { ?>
						<td class="esr-user-note">
							<?php if ( ! empty( $student_note ) ) { ?>
								<span class="dashicons dashicons-admin-comments esr-show-note" title="<?php echo htmlspecialchars( $student_note, ENT_QUOTES, 'UTF-8' ); ?>"></span>
								<span class="dashicons dashicons-welcome-comments esr-hide-note"></span>
								<span class="esr-note-message"><?php echo htmlspecialchars( $student_note, ENT_QUOTES, 'UTF-8' ); ?></span>
							<?php } ?>
						</td>
					<?php } ?>
					<td class="status"><?php echo ESR()->registration_status->get_title( $registration->status ); ?></td>
					<td class="student-name"><?php echo $student_name; ?></td>
					<?php if ( $user_show_emails ) { ?>
						<td class="student-email"><?php echo $student_email; ?></td>
					<?php } ?>
					<?php if ( $show_phone ) { ?>
						<td class="student-phone"><?php echo $student_phone; ?></td>
					<?php } ?>
					<td class="course"><?php echo $registration->course_id . ' - ' . stripslashes( $course_meta->title ) . ' (' . ESR()->day->get_day_title( $course_meta->day ) . ')'; ?></td>
					<td><?php
						if ( $has_multiple_days ) {
							...
							echo ESR()->day->get_day_title( $course_meta->day );
						}
						?></td>
					<td><?php
						if ( $has_multiple_days ) {
							...
							echo $course_meta->time_from . '/' . $course_meta->time_to;
						}
						?></td>
					<td class="dancing-as"
					    data-as="<?php echo $registration->dancing_as; ?>"><?php echo ESR()->dance_as->get_title( $registration->dancing_as ); ?></td>
					<?php if ( $user_show_emails ) { ?>
						<td class="dancing-with"><?php echo $registration->dancing_with; ?></td>
					<?php } ?>
					<td class="partner-name" data-partner="<?php echo $partner_name; ?>"
					    data-email="<?php echo( $user_show_emails ? $partner_email : '' ); ?>"><?php echo $partner_name; ?></td>
					<?php if ( intval( ESR()->settings->esr_get_option( 'free_registrations_enabled', - 1 ) ) === 1 ) { ?>
						<td class="free-registration"><?php echo ESR()->enum_free_registration->get_title( $registration->free_registration ); ?></td>
					<?php } ?>
					<?php if ( $show_payment_enabled ) { ?>
						<td class="payment-status"><?php echo ESR()->payment_status->get_title( $registration->payment_status ); ?></td>
					<?php } ?>
	...
	private function print_action_box( $wave_id ) {
		...
		?>
		<ul class="<?php echo implode( ' ', $class_data['classes'] ) ?>">
			...
			<?php if ( intval( ESR()->settings->esr_get_option( 'free_registrations_enabled', - 1 ) ) != - 1 ) { ?>
				<li class="esr-action update-free-registration">
					<a href="javascript:;">
						...
						<span class="esr-show-status-0"><?php _e( 'Paid Registration', 'easy-school-registration' ); ?></span>
						<span class="esr-show-status-1"><?php _e( 'Free Registration', 'easy-school-registration' ); ?></span>
					</a>
				</li>
			<?php } ?>
			<li class="esr-action confirm">
				<a href="javascript:;">
					...
					<span><?php _e( 'Confirm', 'easy-school-registration' ); ?></span>
				</a>
			</li>
			<li class="esr-action remove">
				<a href="javascript:;">
					<span class="dashicons dashicons-no-alt"></span>
					<span><?php _e( 'Cancel', 'easy-school-registration' ); ?></span>
				</a>
			</li>
			<li class="esr-action remove-forever">
				<a href="javascript:;">
					<span class="dashicons dashicons-trash"></span>
					<span><?php _e( 'Remove Forever', 'easy-school-registration' ); ?></span>
				</a>
			</li>
