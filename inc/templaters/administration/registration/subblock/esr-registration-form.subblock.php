...
class ESR_Registrations_Edit_Form_Subblock_Templater {
	...
	public static function input_student_email() {
		...
		<tr class="esr-student">
			<th><?php _e('Student Email', 'easy-school-registration'); ?></th>
			<td><input required type="email" name="student_email">
				<?php ESR_Tooltip_Templater_Helper::print_tooltip(__('If you want to change the email address, use the Users section in your WordPress. Follow the Documentation on ESR website for more details.', 'easy-school-registration')); ?>
			</td>
		</tr>
		<?php
	}
			...
			<td>
				<select name="course_id">
					...
					foreach (ESR()->course->get_courses_data_by_wave($wave_id) as $id => $course) {
						?>
						<option value="<?php echo $course->id; ?>"><?php echo stripslashes($course->title) . ' - ' . ESR()->day->get_day_title($course->day) . ' ' . $course->time_from . '/' . $course->time_to; ?></option><?php
					}
					?>
				</select>
			</td>
	...
	public static function input_dancing_with() {
		...
		<tr class="esr-registered-partner">
			<th><?php _e('Registered Partner Email', 'easy-school-registration'); ?></th>
			<td><input required type="email" name="dancing_with">
				<?php ESR_Tooltip_Templater_Helper::print_tooltip(__('This is the email address filled out in the registration form. If the Course uses Automatic pairing mode, the system will be waiting for a student with this email address to register and then pair them together.', 'easy-school-registration')); ?>
			</td>
		</tr>
		<?php
	}
	...
	public static function input_partner_email() {
		...
		<tr class="esr-partner-email">
			<th><?php _e('Paired Partner Email', 'easy-school-registration'); ?></th>
			<td><input required type="email" name="partner_email">
				<?php ESR_Tooltip_Templater_Helper::print_tooltip(__('The email address of a Student the system paired this Student with.', 'easy-school-registration')); ?>
			</td>
		</tr>
		<?php
	}
	...
	public static function input_submit() {
		...
		<tr>
			...
			<td>
				...
				<input type="submit" name="esr_registration_edit_submit" value="<?php _e('Save', 'easy-school-registration'); ?>">
			</td>
		</tr>
		<?php
	}
