...
class ESR_Template_Registrations {
	...
	public static function print_page() {
		...
		if (isset($_GET['cin_wave_id'])) {
			$selected_wave = $_GET['cin_wave_id'];
		}

		?>
		<div class="wrap esr-settings-registrations esr-settings"
		     <?php if (isset($_GET['cin_course_id'])) { ?>data-cin-course-id="<?php echo $_GET['cin_course_id']; ?>"<?php } ?>
		     <?php if (isset($_GET['cin_student_id'])) { ?>data-cin-student-id="<?php
		        $student = get_user_by('ID', $_GET['cin_student_id']);
		        if (!empty($student)) {
			...
			<div class="esr-settings-header esr-clearfix">
				<h1 class="wp-heading-inline"><?php _e('Registration', 'easy-school-registration'); ?></h1>
			</div>
			<?php
