...
class ESR_Payments_Templater {
	...
	public static function print_page() {
		...
		$tabs = apply_filters('esr_payments_admin_tabs', [
			'payments' => [
				'title'  => __('Payments', 'easy-school-registration'),
		...
		if (current_user_can('esr_payment_debts_view')) {
			$tabs['debts']    = [
				'title'  => __('Student Debts', 'easy-school-registration'),
			...
			<?php if (intval(ESR()->settings->esr_get_option('debts_enabled', -1)) === -1) {
				...
				<h2 class="nav-tab-wrapper"><?php
					foreach ($tabs as $tab_key => $tab) {
						...
						?>
						<a href="<?php echo esc_url($tab_url); ?>" class="nav-tab<?php echo $active; ?>"><?php echo $tab['title']; ?></a>
						<?php
					}
					?></h2>
