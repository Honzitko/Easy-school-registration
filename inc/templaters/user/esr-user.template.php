...
class ESR_User_Template {
	...
	public static function esr_add_user_fields_callback($user) {
		...
		?>
		<h3><?php _e('Easy School Registration', 'easy-school-registration'); ?></h3>
		<table class="form-table">
			<tr class="user-job-title-wrap">
				<th><label for="esr_phone"><?php _e('Phone Number', 'easy-school-registration'); ?></label></th>
				<td><input type="text" name="esr_phone" id="esr_phone" value="<?php echo esc_attr($user_phone) ?>" class="regular-text ltr"/></td>
			</tr>
			<tr class="user-description-wrap">
				<th><label for="esr_newsletter"><?php _e('Newsletter', 'easy-school-registration'); ?></label></th>
				<td><input type="checkbox" name="esr_newsletter" id="esr_newsletter" value="<?php echo ($user_newsletter !== null) && ($user_newsletter !== '') ? $user_newsletter : current_time('Y-m-d H:i:s'); ?>" id="esr_newsletter" <?php checked(1, ($user_newsletter !== null) && ($user_newsletter !== '')) ?>></td>
			</tr>
		</table>
		<?php
