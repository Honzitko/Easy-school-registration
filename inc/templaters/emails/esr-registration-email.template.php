...
class ESR_Template_Registration_Email {
	...
	public function send_email($wave_ids, $user_id, $registration_data, $user_info) {
		if (intval(ESR()->settings->esr_get_option('registration_email_enabled', -1)) !== -1) {
			...
			$subject = stripcslashes(ESR()->settings->esr_get_option('registration_email_subject'));
			$body = stripcslashes(ESR()->settings->esr_get_option('registration_email_body', null));
					...
					if (isset($tag['parameter'])) {
						switch ($tag['parameter']) {
							...
							case 'checkbox_option':
								{
									$parameter = ((isset($user_info->{$tag['id']}) && (($user_info->{$tag['id']} === '') || !$user_info->{$tag['id']})) || !isset($user_info->{$tag['id']})) ? __('No', 'easy-school-registration') : __('Yes', 'easy-school-registration');
									break;
								}
						}
