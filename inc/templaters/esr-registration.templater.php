...
class ESR_Registration_Templater {
	...
	public function print_courses_registration_form( $default_data, $wave_ids, $show_groups = false ) {
		...
		if ( $registration_open ) {
			...
			<form id="esr-course-registration-form"
			      ...
			      data-show-groups="<?php echo $show_groups; ?>"
			      data-round-payments="<?php echo ESR()->settings->esr_get_option( 'round_payments', - 1 ); ?>"
			      data-no-courses="<?php _e( 'At least one course required to select.', 'easy-school-registration' ); ?>">
				<div class="esr-choosed-courses"
				     data-price-template="<?php echo ESR()->currency->prepare_price( '[price]' ); ?>">
					<div class="esr-header">
						<span><?php _e( 'Course', 'easy-school-registration' ); ?></span>
						<span class="esr-hide-mobile"><?php _e( 'Registration info', 'easy-school-registration' ); ?></span>
						<span><?php _e( 'Price', 'easy-school-registration' ); ?></span>
					</div>
					<?php
					foreach ( $wave_ids as $wave_id ) {
						if ( ESR()->wave->is_wave_registration_active( $wave_id ) ) {
							...
							?>
							<div class="esr-clearfix esr-group wave_<?php echo $wave_id; ?>"><?php

							?>
							<div class="esr-clearfix esr-group-header">
								<?php echo $wave_data->title; ?>
							</div>
							<?php
							?>
							<div class="esr-clearfix esr-group-content"><?php
								if ( $show_groups ) {
									...
									foreach ( $groups as $group_id ) {
										...
										?>
										<div class="esr-clearfix esr-group-<?php echo $group_id['group_id']; ?>">
											<div class="esr-clearfix esr-group-header">
												<?php echo $group; ?>
											</div>
							...
							<?php if ( count( $wave_ids ) > 1 ) { ?>
								<div class="esr-clearfix esr-wave-price esr-footer">
									<div class="esr-price-label"><?php _e( 'Wave price', 'easy-school-registration' ); ?></div>
					...
					<div class="esr-clearfix esr-footer">
						<div class="esr-price-label"><?php _e( 'Final price', 'easy-school-registration' ); ?></div>
				...
				<table class="esr-user-form">
					<tbody>
					<tr>
						<th class="required"><?php _e( 'First Name', 'easy-school-registration' ); ?></th>
						<th class="required"><?php _e( 'Surname', 'easy-school-registration' ); ?></th>
					</tr>
					<tr>
						<td>
							<input required type="text" name="name"
							       value="<?php echo $default_data['name']; ?>">
						</td>
						<td>
							<input required type="text" name="surname"
							       value="<?php echo $default_data['surname']; ?>">
						</td>
					</tr>
					</tbody>
					<tbody>
					<tr>
						<th class="required" <?php if ( ! $is_one_column ) {
							echo 'colspan="2"';
						} ?>><?php _e( 'Email', 'easy-school-registration' ); ?></th>
						<?php
					...
					<tr>
						<td <?php if ( ! $is_one_column ) {
							...
							<input required type="text" name="email"
							       value="<?php echo $default_data['email']; ?>">
						</td>
						<?php
						if ( ! $show_confirm_email_input ) {
						...
						<tr>
							<td colspan="2"><label for="newsletter">
									<input name="newsletter" type="checkbox"
										...
										?>>
									<?php echo nl2br( stripcslashes( ESR()->settings->esr_get_option( 'newsletter_text', '' ) ) ); ?></label>
							</td>
						</tr>
						</tbody>
						<?php
					...
					if ( intval( ESR()->settings->esr_get_option( 'terms_conditions_enabled', - 1 ) ) != - 1 ) {
						...
						<tr>
							<td colspan="2"><label for="terms-conditions">
									...
									<?php echo nl2br( stripcslashes( ESR()->settings->esr_get_option( 'terms_conditions_text', '' ) ) ); ?></label>
							</td>
						</tr>
						</tbody>
						<?php
					...
					<?php if ( intval( ESR()->settings->esr_get_option( 'note_disabled', - 1 ) ) !== 1 ) { ?>
						<tbody>
						<tr>
							<th colspan="2"><?php echo ESR()->settings->esr_get_option( 'registration_note_title', __( 'Note', 'easy-school-registration' ) ); ?></th>
						</tr>
						<tr>
							<?php $note_length = ESR()->settings->esr_get_option( 'note_limit', 0 ); ?>
							<td colspan="2" <?php if ( $note_length > 0 ) { ?>class="esr-textarea-limit"<?php } ?>><textarea name="note" <?php if ( $note_length > 0 ) { ?>maxlength="<?php echo $note_length; ?>"<?php } ?>></textarea></td>
						</tr>
						</tbody>
					<?php } ?>
				</table>
				<p><input type="submit" name="esr-registration-submitted" value="<?php _e( 'Register', 'easy-school-registration' ); ?>"/></p>
			</form>
	...
	private function print_course_registration_row() {
		...
		<div class="esr-course-row course-%course-id%" data-course="%course-id%">
			...
			<div class="mobile-price"><?php echo ESR()->currency->prepare_price( '%course-price%' ); ?></div>
			<div class="registration-info">
				<div class="esr-info-row">
					<span class="esr-info-row-label"><?php _e( 'Dancing as', 'easy-school-registration' ); ?>:</span>
					<?php
					...
					<select class="esr-info-row-input esr-dancing-as" required
						...
						<option value=""><?php echo __( '- choose -', 'easy-school-registration' ); ?></option>
						<option value="<?php echo ESR_Dancing_As::LEADER; ?>"><?php echo( empty( $leader_settings_label ) ? ESR()->dance_as->get_title( ESR_Dancing_As::LEADER ) : $leader_settings_label ); ?></option>
						<option value="<?php echo ESR_Dancing_As::FOLLOWER; ?>"><?php echo( empty( $follower_settings_label ) ? ESR()->dance_as->get_title( ESR_Dancing_As::FOLLOWER ) : $follower_settings_label ); ?></option>
					</select>
				</div>
				<?php if ( intval( ESR()->settings->esr_get_option( 'dancing_with_enforce', 1 ) ) !== - 1 ) { ?>
					<div class="esr-info-row esr-row-choose-partner">
						<span class="esr-info-row-label"><?php _e( 'Do you have partner?', 'easy-school-registration' ); ?></span>
						<div class="esr-info-row-input">
							<label class="esr-choose-partner"><input type="radio" class="choose_partner" name="choose-partner-%course-id%" value="1" required> <?php _e( 'Yes', 'easy-school-registration' ); ?></label>
							<label class="esr-choose-partner"><input type="radio" class="choose_partner" name="choose-partner-%course-id%" value="0" required> <?php _e( 'No', 'easy-school-registration' ); ?></label>
						</div>
					</div>
					<div class="esr-info-row esr-row-dancing-with" style="display: none;">
						<span class="esr-info-row-label"><?php _e( 'Partner email', 'easy-school-registration' ); ?>:</span>
						<input class="esr-info-row-input esr-dancing-with" type="email" name="dancing-with">
					</div>
				<?php } ?>
			</div>
			<div class="price esr-hide-mobile" data-price="%course-price%"><?php echo ESR()->currency->prepare_price( '%course-price%' ); ?></div>
	...
	private function esr_print_phone_header( $show_phone_input ) {
		if ( $show_phone_input ) { ?>
			<th class="<?php if ( intval( ESR()->settings->esr_get_option( 'phone_required', 1 ) ) !== - 1 ) {
				echo 'required';
			} ?>"><?php _e( 'Phone', 'easy-school-registration' ); ?></th>
		<?php }
	}
		...
		if ( $show_phone_input ) { ?>
			<td>
				<input type="text" name="phone"
				       value="<?php echo( $default_data['phone'] ? $default_data['phone'] : '' ); ?>"
	...
	private function esr_print_reconfirm_email_header() {
		?>
		<th class="required"><?php _e( 'Confirm Email', 'easy-school-registration' ); ?></th>
		<?php
	}
	...
	private function esr_print_confirm_email_input() {
		...
		<td>
			<input class="esr-confirm-email" required type="text" name="esr-confirm-email"
			       data-error-message="<?php _e( 'Emails are not same', 'easy-school-registration' ); ?>">
		</td>
		<?php
	}
