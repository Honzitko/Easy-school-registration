...
class ESR_User {
	...
	public function process_user_registration($data) {
		...
		$phone   = strtolower(isset($data->phone) ? htmlspecialchars(trim($data->phone)) : NULL);
		$user_id = null;
		...
		if (email_exists($email)) {
			...
			$user_id = $this->create_new_user($email, htmlspecialchars($data->name), htmlspecialchars($data->surname))->ID;
		}
