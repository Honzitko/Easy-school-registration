...
class ESR_Payment {
	...
	public function get_payments_by_wave($wave_id, $object = ARRAY_A) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}esr_user_payment WHERE wave_id = %d ORDER BY id", [$wave_id]), $object);
	}
	...
	public function get_payments_by_user($user_id, $object = OBJECT_K) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}esr_user_payment WHERE user_id = %d ORDER BY id", [$user_id]), $object);
	}
	...
	public function get_payments_for_export($user_id) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT up.*, wd.title AS wave_name FROM {$wpdb->prefix}esr_user_payment AS up JOIN {$wpdb->prefix}esr_wave_data AS wd ON wd.id = up.wave_id WHERE up.user_id = %d ORDER BY id", [$user_id]), OBJECT_K);
	}
	...
	public function get_payment_by_wave_and_user($wave_id, $user_id) {
		...
		return $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}esr_user_payment WHERE user_id = %d AND wave_id = %d ORDER BY id", [(int) $user_id, (int) $wave_id]));
	}
	...
	public function get_partially_paid_users_for_payment_emails($wave_id) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT user_id,  u.display_name, u.user_email, up.note, up.payment, up.to_pay FROM {$wpdb->prefix}esr_user_payment AS up JOIN {$wpdb->users} AS u ON up.user_id = u.id WHERE up.status = %d AND up.payment < up.to_pay AND up.wave_id = %d GROUP BY up.user_id", [ESR_Enum_Payment::PAID, $wave_id]));
	}
