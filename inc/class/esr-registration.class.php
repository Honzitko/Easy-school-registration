...
class ESR_Registration {
	...
	public function get_confirmed_registrations_by_course($course_id) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT cr.* FROM {$wpdb->prefix}esr_course_registration AS cr WHERE cr.course_id = %d AND cr.status = %d", [$course_id, ESR_Registration_Status::CONFIRMED]));
	}
	...
	public function get_confirmed_registration_user_ids_by_wave($wave_id) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT DISTINCT cr.user_id AS user_id FROM {$wpdb->prefix}esr_course_registration AS cr JOIN {$wpdb->prefix}esr_course_data AS cd ON cr.course_id = cd.id WHERE cd.wave_id = %d AND cr.status = %d", [$wave_id, ESR_Registration_Status::CONFIRMED]));
	}
	...
	public function get_registrations_by_user($user_id) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT cr.*, cd.title AS course_name, wd.title AS wave_name FROM {$wpdb->prefix}esr_course_registration AS cr JOIN {$wpdb->prefix}esr_course_data AS cd ON cr.course_id = cd.id  JOIN {$wpdb->prefix}esr_wave_data AS wd ON cd.wave_id = wd.id  WHERE cr.user_id = %d", [$user_id]), OBJECT_K);
	}
	...
	public function get_confirmed_registrations_by_wave_and_user($wave_id, $user_id) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT *, cr.id AS registration_id FROM {$wpdb->prefix}esr_course_registration AS cr JOIN {$wpdb->prefix}esr_course_data AS cd ON cr.course_id = cd.id WHERE cd.wave_id = %d AND cr.user_id = %d AND cr.status = %d", [$wave_id, $user_id, ESR_Registration_Status::CONFIRMED]));
	}
