...
WHERE up.wave_id = %d AND up.confirmation_email_sent_timestamp IS NOT NULL AND is_paying = 1 AND DATEDIFF(%s, up.confirmation_email_sent_timestamp) >= %d AND up.status = %d";
		...
		$params = [$wave_id, ESR_Registration_Status::CONFIRMED, $wave_id, current_time('Y-m-d'), intval(ESR()->settings->esr_get_option('payment_reminder_email_days', 5)), ESR_Enum_Payment::NOT_PAID];
