	...
	public static function esr_run_waiting_email_callback() {
		...
		foreach ($waves as $key => $wave) {
			...
			if (!empty($update_registrations)) {
				...
				$wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}esr_course_registration AS cr JOIN {$wpdb->prefix}esr_course_data AS cd ON cr.course_id = cd.id SET waiting_email_sent_timestamp = %s WHERE user_id IN ({$update_registrations_string}) AND cd.wave_id = %d AND cr.status = %d", [$today, $wave->id, ESR_Registration_Status::WAITING]));
			}
		}
