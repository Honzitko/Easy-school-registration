...
class ESR_Course
{
	...
	{
		global $wpdb;
		$data = $wpdb->get_results($wpdb->prepare("SELECT cd.* FROM {$wpdb->prefix}esr_course_registration AS cr JOIN {$wpdb->prefix}esr_course_data AS cd ON cr.course_id = cd.id WHERE cd.is_passed = %d AND cd.wave_id = %d AND cr.user_id = %d ORDER BY cd.wave_id DESC", [(int)true, $wave_id, $user_id]));
	...
	public function is_course_already_registered($course_id, $user_id)
	{
		...
		return (boolean)$wpdb->query($wpdb->prepare("SELECT 1 FROM {$wpdb->prefix}esr_course_registration WHERE course_id = %d AND user_id = %d", [$course_id, $user_id]));
	}
	...
	public function is_course_solo($course_id)
	{
		...
		return (boolean)$wpdb->query($wpdb->prepare("SELECT 1 FROM {$wpdb->prefix}esr_course_data WHERE id = %d AND is_solo = %d", [$course_id, 1]));
	}
	...
	public function get_course_price($course_id, $wave_id)
	{
		...
		return $wpdb->get_var($wpdb->prepare("SELECT " . $this->esr_get_course_price_sql($wave_id) . " AS price FROM {$wpdb->prefix}esr_course_data WHERE id = %d", [$course_id]));
	}
	...
	public function is_course_enabled($course_id)
	{
		if ($this->is_course_solo($course_id)) {
			...
			return ESR()->dance_as->is_leader_registration_enabled($course_id) || ESR()->dance_as->is_followers_registration_enabled($course_id);
		}
	}
	...
	public function is_partner_enforcement_enabled($course_id)
	{
		...
		return (boolean)$wpdb->get_var($wpdb->prepare("SELECT enforce_partner FROM {$wpdb->prefix}esr_course_data AS cd WHERE cd.id = %d", [$course_id]));
	}
